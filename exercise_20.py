...
# Define the topic similarity evaluator using Jaccard similarity
def topic_similarity_evaluator(
    inputs: dict, outputs: dict, reference_outputs: dict
):
    # Extract topics from outputs and reference_outputs using list comprehension
    predicted_topics = set(
        opinion["topic"] for opinion in outputs["opinions"]
    )
    reference_topics = set(
        opinion["topic"] for opinion in reference_outputs["structured_review"]["opinions"]
    )

    # Calculate Jaccard similarity: |intersection| / |union|
    intersection = predicted_topics & reference_topics
    union = predicted_topics | reference_topics
...
# Define evaluators
evaluators = [
    # 1. Topic similarity using Jaccard
