...
class Opinion(BaseModel):
    topic: str = Field(
        # description='The topic of the opinion'
...
def get_response(prompt_template_str, review, output_structure, input_structure):
    prompt_template = PromptTemplate.from_template(prompt_template_str)
    prompt = prompt_template.format(review=review, output_structure=output_structure, input_structure=input_structure)
...
def extract_structured_review(review_data, return_prompt=False):
    ...
    input_structure = Review.model_json_schema()
    ...
    structured_review, prompt = get_response(
        prompt_template_str, review_str, output_structure, input_structure
    )
